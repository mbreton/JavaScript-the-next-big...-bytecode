Reveal.initialize({
    ...
    dependencies: [
        { src: 'plugin/highlight/highlight.min.js', async: true, callback: function () {
            var codeToLoad = {
                'ut-editor': 'examples/ut.js',
                'ts-jquery': 'examples/jquery.ts',
                'coffee-oo': 'examples/coffee-oo.coffee',
                'sugar1': 'examples/sugar1.coffee',
                'sugar2': 'examples/sugar2.coffee',
                'sugar3': 'examples/sugar3.coffee',
                'dart-type': 'examples/type.dart',
                'dart-class': 'examples/dart_class.dart',
                'dart-futur': 'examples/dart_futur.dart',
                'dart-stream': 'examples/dart_stream.dart',
                'dart-isolate': 'examples/dart_isolate.dart',
                'asm-simple': 'examples/asm-simple.js',
                'asm-call': 'examples/asm-call.js',
                'dart-type-string': 'examples/dart_type_string.dart',
                'webserver': 'examples/webserver.js',
                'func_js': 'examples/func_js.js',
                'context-js': 'examples/context.js',
                'context-js-result': 'examples/context_result.js'
            };
            var promisesOfCode = [];
            for (var currentDomId in codeToLoad) {
                var promise = $.ajax({
                    url: codeToLoad[currentDomId],
                    dataType: 'text'
                });
                (function (domId) {
                    promise.then(function (content) {
                        console.log(codeToLoad[domId] + ' loaded');
                        if (domId.indexOf('editor') !== -1) {
                            ace.edit(domId).setValue(content, -1);
                        } else {
                            $('#' + domId).text(content);
                        }
                    });
                    promisesOfCode.push(promise);
                })(currentDomId);
            }
            $.when.apply($, promisesOfCode).done(function () {
...
function setTitle(event) {
    var slideIdx = event.indexh;
    if (slideIdx < 11) {
        $('#rotate45 h2').text("L'apogée");
        $('#rotate45 h2').attr('class', 'text-white');
    }
    else if (slideIdx <= 15) {
        $('#rotate45 h2').text("L'avenir");
        $('#rotate45 h2').attr('class', 'text-white');
    }
    else if (slideIdx <= 22) {
        $('#rotate45 h2').text("TypeScript");
        $('#rotate45 h2').attr('class', 'ts');
    }
    else if (slideIdx <= 31) {
        $('#rotate45 h2').text("CoffeeScript");
        $('#rotate45 h2').attr('class', 'coffee');
    }
    else if (slideIdx <= 45) {
        $('#rotate45 h2').text("Dart");
        $('#rotate45 h2').attr('class', 'dart');
    }
    else if (slideIdx <= 58) {
        $('#rotate45 h2').text("ASM.js");
        $('#rotate45 h2').attr('class', 'asm');
    }
    else {
        $('#rotate45 h2').text("So...");
        $('#rotate45 h2').attr('class', '')
    }
}
function initNoteInConsole() {
    ...
    coffeeEditor.on('change', function () {
        ...
        setTimeout(function () {
            try {
                ...
                jsRendering.getSession().setValue(e.stack);
            }
        }, 1);
    });
}
...
function initDartCompiler() {
    var editor = ace.edit('dart-src');
    var jsRendering = ace.edit('dart-dest');

    editor.on('change', _.throttle(function () {
        var dartCode = editor.getSession().getValue();

        try {
            jsRendering.getSession().setValue('//compiling ...');
            $.ajax({
                url: '/dart',
                type: 'POST',
                dataType: 'text',
                ...
                jsRendering.getSession().setValue('// compilation failed !');
            });
        } catch (e) {
...
function initAsmCompiler() {
    var editor = ace.edit('asm-src');
    var jsRendering = ace.edit('asm-dest');

    editor.on('change', _.throttle(function () {
        var cppCode = editor.getSession().getValue();
...
function initTypeScriptCompiler() {
    ...
    function compileTypeScript(editor, renderer) {
        ...
        setTimeout(function () {
            ...
            renderer.getSession().setValue(outfile.source);
        }, 0);
    }


    var typeScriptEditor = ace.edit('typescript-src');
    var jsRenderer = ace.edit('typescript-dest');
    ...
    function loadFile(path, title) {
        return function () {
            $.get(path, function (content) {
                typeScriptEditor.setValue(content, -1);
            });
            $("#ts-editor-title").html(title);
            return false;
        }
    }

    var loadJs = loadFile('examples/js.ts', '<span class="color">Super ensemble</span> de JavaScript');
    //loadJs();

    function onFragment(event) {
        if (event.fragment == $('#tsc-js')[0]) {
            loadJs();
        } else if (event.fragment == $("#tsc-module")[0]) {
            loadFile('examples/module.ts', '<span class="color">Modularisation</span> du code')();
        } else if (event.fragment == $("#tsc-oo")[0]) {
            loadFile('examples/oo.ts', 'Modèle <span class="color">orienté objet</span>')();
        } else if (event.fragment == $("#tsc-generics")[0]) {
            loadFile('examples/generic.ts', 'Les <span class="color">génériques</span>')();
        }
    }

    Reveal.addEventListener('fragmentshown', onFragment);
    Reveal.addEventListener('fragmenthidden', onFragment);

    $('.editor-links a').on('click', function (e) {
